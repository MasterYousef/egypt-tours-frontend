...
import { ErrorResponse, tour, tourForm } from "@/app/types/types";
...
const createTourLogic = (token:string) => {
  ...
  const handleSubmit = async (
    ...
    if (
      ...
      event.target.duration.value === "" || img === im.src
    ) {
      ...
      data.current.set("start", selected.toLocaleDateString('en-GB').split("/").reverse().join("-"));
      data.current.set("duration", event.target.duration.value);
      const res = await useInsertData<tour>("/api/v1/tour", data.current,token);
